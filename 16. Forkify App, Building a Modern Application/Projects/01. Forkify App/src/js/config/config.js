export const API_URL = `https://forkify-api.herokuapp.com/api/v2/recipes`;
export const REQUEST_TIMEOUT = 5;
export const PAGE_RESULTS_LIMIT = 10;
export const MAX_INGREDIENTS_COUNT = 20;
export const MIN_INGREDIENTS_COUNT = 1;
export const MAX_STEPS_COUNT = 10;
export const MIN_STEPS_COUNT = 1;
export const RECIPE_HISTORY_LENGTH = 10;
export const SEARCH_HISTORY_LENGTH = 1;
export const PROMPT_MIN_LENGTH = 2;
export const FEED_MAX_RESULTS = 70;
export const FEED_MAX_TOPICS = 3;
export const COMMUNITY_MAX_TOPICS = 5;
export const FEED_UPDATE_MILLISECONDS = 86_400_000;
export const COMMUNITY_UPDATE_MILLISECONDS = 172_800_000;
export const DEFAULT_SORT_FUNCTION = 0;
